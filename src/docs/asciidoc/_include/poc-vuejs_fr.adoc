...
.. copie du fichier docs/asciidoc/images/PortalPageDetail-Example.svg dans ofbiz-framework
.. copie du fichier docs/asciidoc/developer-manual_fr.adoc dans ofbiz-framework
.. ajout du champ watcherName dans la définition de la table PortalPagePortlet: +
  Dans le fichier framework/common/entitydef/entitymodel.xml à la ligne 824 vous avez la définition de la table
  "PortalPagePortlet" et il faut ajouter le champ watcherName en dernier champ. +
  Pour vous aider vous pouvez lire ou appliquer le patch entitymodel.xml.patch qui se trouve dans le répertoire ofbizfile du plugin vuejsPortal 
  dans framework/common/entitydef/
