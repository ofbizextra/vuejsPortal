...
* form
  ...
  ** in `<hyperlink` tag
    ...
    *** `url-mode` two new modes is added to be able to give the http method to use, POST is the default
...
* menu
  ** in `<link` tag
    ...
    *** `url-mode` two new modes is added to be able to give the http method to use, POST is the default (except for setArea)
      **** intra-put : do the http request with a PUT method
      **** intra-delete : do the http request with a DELETE method
