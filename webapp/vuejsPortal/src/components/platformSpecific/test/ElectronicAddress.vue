<template>
  <v-layout wrap xs12>
    <v-flex text-left stretch xs12>
      <h4 class="mb-2">Electronic Address - ({{props.length}})</h4>
      <v-card>
        <v-simple-table dense text-right dense>
          <thead>
          <tr>
            <th class="col-10">Electronic Address</th>
          ...
          <transition-group tag="tbody" name="test">
            <tr class="xs-12" v-for="(email, id) in props" :key="email.contactMech.contactMechId">
              ...
              <td>
                <v-btn-toggle>
                  <v-btn color="primary" @click="updateEmail(email)">
                    <v-icon>mdi-pencil</v-icon>
                  </v-btn>
                  <v-btn color="error" @click="deleteEmail(email)">
...
<script>
  const updateContactMech = 'https://localhost:8443/partymgrapi/control/updateEmailAddress'
  ...
  export default {
    ...
    methods: {
      deleteEmail(email) {
        this.$http.post(deleteContactMech, {partyId: 'DemoLead3', contactMechId: email.contactMech.contactMechId}).then(
          result => {
      ...
      updateEmail(email) {
        this.$http.post(updateContactMech, {partyId: 'DemoLead3', contactMechId: email.contactMech.contactMechId}).then(
          result => {
            console.log(result.body)
          },
          error => {
